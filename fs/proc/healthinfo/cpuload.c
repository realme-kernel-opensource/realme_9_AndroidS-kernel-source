...
int ohm_get_cur_cpuload(bool ctrl)
{
	...
	clock_t ct_user, ct_system, ct_idle, ct_iowait, ct_irq, ct_softirq, load,
		sum = 0;
	...
	ct_user = nsec_to_clock_t(cpu_load.t_user) - nsec_to_clock_t(
			  cpu_load_temp.t_user);
	ct_system = nsec_to_clock_t(cpu_load.t_system) - nsec_to_clock_t(
			    cpu_load_temp.t_system);
	ct_idle = nsec_to_clock_t(cpu_load.t_idle) - nsec_to_clock_t(
			  cpu_load_temp.t_idle);
	ct_iowait = nsec_to_clock_t(cpu_load.t_iowait) - nsec_to_clock_t(
			    cpu_load_temp.t_iowait);
	ct_irq = nsec_to_clock_t(cpu_load.t_irq) - nsec_to_clock_t(cpu_load_temp.t_irq);
	ct_softirq = nsec_to_clock_t(cpu_load.t_softirq) - nsec_to_clock_t(
