	...
	if (!list_empty(&rq->ux_thread_list)) {
		...
		if (key_task) {
			...
			if (key_se && (rq->clock >= key_task->enqueue_time) &&
					rq->clock - key_task->enqueue_time >= ((u64)ux_min_sched_delay_granularity *
							S2NS_T)) {
				*p = key_task;
				*se = key_se;
			}
				   ...
				   bool boost, bool prefer_idle, unsigned int type)
{
	...
	for_each_cpu(i, cpu_active_mask) {
		...
		if ((curr->prio > lowest_prio) || (boost
						   && (capacity_curr > lowest_prio_max_cap))) {
		      ...
		      struct task_struct *ux_task, int reset_type)
{
	...
	inherit_task->ux_state = (inherit_task->ux_state & ~SCHED_ASSIST_UX_MASK) |
				 reset_inherit;
				  ...
				  size_t count, loff_t *ppos)
{
	...
	len = snprintf(buffer, sizeof(buffer),
		       "pid=%d ux_state=%d inherit=%llx(fu:%d mu:%d rw:%d bi:%d)\n",
		       task->pid, ux_state, task->inherit_ux,
		       test_inherit_ux(task, INHERIT_UX_FUTEX), test_inherit_ux(task,
				       INHERIT_UX_MUTEX),
		       test_inherit_ux(task, INHERIT_UX_RWSEM), test_inherit_ux(task,
				       INHERIT_UX_BINDER));
