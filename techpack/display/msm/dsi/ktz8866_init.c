...
int lcd_bl_enable(bool enable)
{
	...
	if (enable) {
		lcd_bl_write_byte(0x02, 0xD3);/* BL_CFG1；OVP=34V，指数调光，PWM enable */
		...
		lcd_bl_write_byte(0x15, 0xB1);/* Backlight Full-scale LED Current 24.4mA/CH；*/
...
int lcd_bl_set_led_brightness(int value)//for set bringhtness
{
	...
	pr_info("%s: bl = %d,mapping value = %d\n", __func__, value, backlight_buf[value]);
	if (value > 0) {
...
{
	printk("--lcd, samir enter lcd_disable_bias function,value = %d", enable);
	if (enable) {
...
int turn_on_ktz8866_hw_en(bool on)
{
	int ret;
	u8 value  = on ? 1 : 0;
	printk("--lcd, samir enter turn_on_ktz8866_hw_en function,value = %d", value);
...
int turn_on_panel_vio_en(bool on)
{
	int ret;
	u8 value  = on ? 1 : 0;
	printk("--lcd, samir enter turn_on_panel_vio_en function,value = %d", value);
