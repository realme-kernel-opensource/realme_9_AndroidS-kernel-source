	...
	enum cci_i2c_master_t master)
{
	...
	if (!cci_dev->cci_master_info[master].is_initilized) {
		...
		cci_dev->cci_i2c_queue_info[master][QUEUE_0].report_id = 0;
		cci_dev->cci_i2c_queue_info[master][QUEUE_1].report_id = 0;
	}
	...
	struct cam_cci_ctrl *c_ctrl)
{
	...
	if (cci_dev->ref_count++) {
		...
		CAM_DBG(CAM_CCI, "ref_count %d, master: %d cci%d",
			cci_dev->ref_count, master,  cci_dev->soc_info.index);
		return rc;
	}
	...
	enum cci_i2c_master_t master)
{
	...
	if (!(--cci_dev->master_active_slave[master])) {
		...
		CAM_DBG(CAM_CCI,
			"All submodules are released for master: %d cci%d",
				master, cci_dev->soc_info.index);
	}

	if (--cci_dev->ref_count) {
		CAM_DBG(CAM_CCI, "Submodule release: Ref_count: %d cci%dm%d",
			cci_dev->ref_count, cci_dev->soc_info.index, master);
		return 0;
	}
