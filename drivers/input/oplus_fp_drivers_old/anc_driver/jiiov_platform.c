...
static int anc_opticalfp_tp_handler(struct fp_underscreen_info *tp_info)
{
	...
	pr_info("[anc] Netlink touch info area:%d,x:%d,y:%d", netlink_msg.area_rate,
		netlink_msg.x, netlink_msg.y);
...
static int anc_gpio_init(struct device *dev, struct anc_data *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(data->pinctrl_state); i++) {
		const char *n = pctl_names[i];
		struct pinctrl_state *state = pinctrl_lookup_state(data->fingerprint_pinctrl,
					      n);
...
static int anc_irq_init(struct device *dev, struct anc_data *data)
{
	...
	rc = devm_request_threaded_irq(dev, data->irq, NULL, anc_irq_handler, irqf,
				       dev_name(dev), data);
