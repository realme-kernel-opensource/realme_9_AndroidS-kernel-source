...
long egistec_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case INT_TRIGGER_INIT:
		...
		retval = Interrupt_Init(egistec, data.int_mode, data.detect_period,
...
int egistec_parse_dt(struct device *dev, struct egistec_data *data)
{
	...
	if (node) {
		...
		if (pdev) {
			...
			data->pstate_spi_6mA = pinctrl_lookup_state(data->pinctrl_gpios,
					       "gf_spi_drive_6mA");
			...
			data->pstate_spi_6mA_pull_low = pinctrl_lookup_state(data->pinctrl_gpios,
