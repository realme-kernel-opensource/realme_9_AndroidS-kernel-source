...
int fpsensor_spidev_dts_init(fpsensor_data_t *fpsensor)
{
	...
	if (node) {
		...
		fpsensor->fp_rst_low = pinctrl_lookup_state(fpsensor->pinctrl1,
				       "fpsensor_finger_rst_low");
		...
		fpsensor->fp_rst_high = pinctrl_lookup_state(fpsensor->pinctrl1,
...
static int fpsensor_dev_setup(fpsensor_data_t *fpsensor)
{
	...
	if (fpsensor_dev_major) {
		...
		ret = alloc_chrdev_region(&dev_no, fpsensor_dev_minor, FPSENSOR_NR_DEVS,
					  FPSENSOR_DEV_NAME);
	...
	dev = device_create(fpsensor->class, &fpsensor->spi->dev, dev_no, fpsensor,
			    FPSENSOR_DEV_NAME);
...
#endif
{
	...
	if (CHIPONE_LDO_DISABLE == 1) {
		fpsensor_dev->fp_pwr_high = pinctrl_lookup_state(fpsensor_dev->pinctrl1,
					    "power_high");
		...
		fpsensor_dev->fp_pwr_low = pinctrl_lookup_state(fpsensor_dev->pinctrl1,
					   "power_low");
