			   ...
			   int data_type)
{
	...
	ret = touch_i2c_read_block(ts_data->client, raw_addr,
				   byte_num > MAX_PACKET_SIZE ? MAX_PACKET_SIZE : byte_num, buf);
			    ...
			    struct i2c_client *client)
{
	...
	if (ts_data->reset_pinctrl) {
		...
		if (IS_ERR_OR_NULL(ts_data->hw_res->pinctrl)) {
			...
			ts_data->hw_res->pin_rst2_high = pinctrl_lookup_state(ts_data->hw_res->pinctrl,
							 "pin_rst2_high");
			...
			ts_data->hw_res->pin_rst2_low = pinctrl_lookup_state(ts_data->hw_res->pinctrl,
							"pin_rst2_low");
			...
			ts_data->hw_res->pin_hw1_high = pinctrl_lookup_state(ts_data->hw_res->pinctrl,
							"pin_hw1_high");
			...
			ts_data->hw_res->pin_hw1_low = pinctrl_lookup_state(ts_data->hw_res->pinctrl,
						       "pin_hw1_low");
			...
			ts_data->hw_res->pin_hw2_high = pinctrl_lookup_state(ts_data->hw_res->pinctrl,
							"pin_hw2_high");
			...
			ts_data->hw_res->pin_hw2_low = pinctrl_lookup_state(ts_data->hw_res->pinctrl,
						       "pin_hw2_low");
