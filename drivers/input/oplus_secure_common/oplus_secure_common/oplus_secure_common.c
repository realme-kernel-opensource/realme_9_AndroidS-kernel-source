...
static int secure_common_parse_parent_dts(struct secure_data *secure_data)
{
	...
	dev_info(secure_data->dev,
		 ...
		 secure_data->sec_override1_reg, secure_data->override1_en_value);
...
static int secure_register_proc_fs(struct secure_data *secure_data)
{
	...
	pentry = proc_create("CryptoKeyUnsupport", 0444, oplus_secure_common_dir,
