...
static int vreg_setup(struct fp_dev *fp_dev, fp_power_info_t *pwr_info,
		      bool enable)
{
	int               rc;
	...
	if (enable) {
		...
		if (regulator_count_voltages(vreg) > 0) {
			rc =
				regulator_set_voltage(vreg, pwr_info->vreg_config.vmin,
...
int fp_parse_pwr_list(struct fp_dev *fp_dev)
{
	...
	for_each_available_child_of_node(np, child) {
		...
		case FP_POWER_MODE_GPIO:
			...
			/* get gpio by name */
			fp_dev->pwr_list[child_node_index].pwr_gpio = of_get_named_gpio(np, node_name,
					0);
