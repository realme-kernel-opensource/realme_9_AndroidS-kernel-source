...
static ssize_t ree_spi_sync(struct spidev_data *spidev,
...
static inline ssize_t spi_sync_read_internal(struct spidev_data *spidev,
		char *tx_buf, char *rx_buf, size_t len)
{
	int ret = 0;
...
static ssize_t fingerprint_read(struct file *f, char __user *buf, size_t count,
				loff_t *offset)
{
...
static ssize_t fingerprint_write(struct file *filp, const char __user *buf,
...
int opticalfp_irq_handler(struct fp_underscreen_info *tp_info)
{
	...
	if (1 == tp_info->touch_state) {
		send_fingerprint_msg(E_FP_TP, tp_info->touch_state, tp_info,
...
static long fingerprint_ioctl(struct file *filp, unsigned int cmd,
			      unsigned long arg)
{
	int err = 0;
	int retval = 0;
...
static int fingerprint_driver_probe(struct spi_device *spi)
{
	...
	if (minor < N_SPI_MINORS) {
		...
		dev = device_create(fingerprint_class, &spi->dev, spidev->devt, spidev,
				    FP_DEV_NAME);
