...
static int irq_setup(struct fp_dev *fp_dev)
{
	...
	status      = request_threaded_irq(
			      fp_dev->irq, NULL, fp_irq_handler, IRQF_TRIGGER_RISING | IRQF_ONESHOT,
			      "oplusfp", fp_dev);
...
static int oplus_fb_notifier_call(struct notifier_block *nb, unsigned long val,
				  void *data)
{
...
#if defined(CONFIG_OPLUS_FINGERPRINT_GKI_ENABLE)
static int oplus_tp_notifier_call(struct notifier_block *nb, unsigned long val,
				  void *data)
{
...
static int fp_probe(oplus_fp_device *pdev)
{
	...
	if (minor < N_SPI_MINORS) {
		...
		dev    = device_create(fp_class, &fp_dev->pdev->dev, fp_dev->devt, fp_dev,
				       FP_DEV_NAME);
	...
	if (status == 0) {
		...
		for (count = 0; count < ARRAY_SIZE(fp_key_maps); count++)
			input_set_capability(fp_dev->input, fp_key_maps[count].type,
					     fp_key_maps[count].code);
