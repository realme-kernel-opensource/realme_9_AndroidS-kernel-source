			   ...
			   u8 param_size)
{
	...
	if (param_offset >= buff_len ||
		...
		dev_err(hba->dev,
			"%s: Invalid offset 0x%x or size 0x%x in descriptor IDN 0x%x, length 0x%x\n",
		...
		struct ufshcd_lrb *lrbp)
{
	if (lrbp->cmd->cmnd[0] == WRITE_10 || lrbp->cmd->cmnd[0] == WRITE_16) {
		if (hba->pwr_info.gear_tx == 1) {
			hba->ufs_transmission_status.gear_min_write_sec += blk_rq_sectors(
						lrbp->cmd->request);
			hba->ufs_transmission_status.gear_min_write_us +=
		...
		if (hba->pwr_info.gear_tx == 3 || hba->pwr_info.gear_tx == 4) {
			hba->ufs_transmission_status.gear_max_write_sec += blk_rq_sectors(
						lrbp->cmd->request);
			hba->ufs_transmission_status.gear_max_write_us +=
				ktime_us_delta(lrbp->compl_time_stamp, lrbp->issue_time_stamp);
		}
		...
		if (hba->pwr_info.gear_rx == 1) {
			hba->ufs_transmission_status.gear_min_read_sec += blk_rq_sectors(
						lrbp->cmd->request);
			hba->ufs_transmission_status.gear_min_read_us +=
				ktime_us_delta(lrbp->compl_time_stamp, lrbp->issue_time_stamp);
		}
		...
		if (hba->pwr_info.gear_rx == 3 || hba->pwr_info.gear_rx == 4) {
			hba->ufs_transmission_status.gear_max_read_sec += blk_rq_sectors(
						lrbp->cmd->request);
			hba->ufs_transmission_status.gear_max_read_us +=
				ktime_us_delta(lrbp->compl_time_stamp, lrbp->issue_time_stamp);
		}
		...
		if (hba->pwr_info.gear_rx == 1) {
			hba->ufs_transmission_status.gear_min_other_sec += blk_rq_sectors(
						lrbp->cmd->request);
			hba->ufs_transmission_status.gear_min_other_us += ktime_us_delta(
						lrbp->compl_time_stamp, lrbp->issue_time_stamp);
		}

		if (hba->pwr_info.gear_rx == 3 || hba->pwr_info.gear_rx == 4) {
			hba->ufs_transmission_status.gear_max_other_sec += blk_rq_sectors(
						lrbp->cmd->request);
			hba->ufs_transmission_status.gear_max_other_us += ktime_us_delta(
						lrbp->compl_time_stamp, lrbp->issue_time_stamp);
		}
	}
...
//Jinghua.Yu@BSP.Storage.UFS 2020/06/12, Add t for ufs transmission_status for midas
			...
			if (cmd->request) {
				...
				u_int64_t delta_us = ktime_us_delta(lrbp->compl_time_stamp,
								    lrbp->issue_time_stamp);
...
int __attribute__((weak)) register_device_proc_for_ufsplus(char *name,
		int *hpb_status, int *tw_status)
{
	return 0;
}
#endif
...
out:
	...
	register_device_proc_for_ufsplus("ufsplus_status", &ufsplus_hpb_status,
					 &ufsplus_tw_status);
#endif
#endif
	return ret;
