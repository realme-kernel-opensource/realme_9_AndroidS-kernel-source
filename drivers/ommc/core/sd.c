...
{
	return mmc_card_uhs(card) ? STR_SPEED_UHS : (mmc_card_hs(
				card) ? STR_SPEED_HS : "");
}
...
static void driver_send_command24(struct sdhci_host *host, u32 *cfg_data,
				  int data_len)
{
	struct mmc_host *mmc = host->mmc;
...
void bht_update_cfg(struct mmc_host *mmc_host, struct mmc_card *card,
	...
	if (status & R1_CARD_IS_LOCKED) {
		if (card->auto_unlock) {
			if (card->unlock_pwd[0] > 0) {
				/* unlock sd card */
				err = mmc_lock_unlock_by_buf(card,  card->unlock_pwd + 1,
							     (int)card->unlock_pwd[0], MMC_LOCK_MODE_UNLOCK);
