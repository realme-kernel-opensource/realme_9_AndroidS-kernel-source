...
int sd_send_lock_unlock_cmd(struct mmc_card *card, u8 *data_buf, int data_size,
			    int max_buf_size, int mode)
{
	int err;
	...
	if ((unsigned int)mode & MMC_LOCK_MODE_ERASE) {
		...
		data.timeout_clks =
			8000000;//clk is 400000,8000000 is 20S,calculate in sdhci_target_timeout
...
int mmc_lock_unlock_by_buf(struct mmc_card *card, u8 *key_buf, int key_len,
			   int mode)
{
...
sdlock_retry:
	...
	printk("[SDLOCK] %s bus_resume_flags = %d", __func__,
	       card->host->bus_resume_flags);
#endif
