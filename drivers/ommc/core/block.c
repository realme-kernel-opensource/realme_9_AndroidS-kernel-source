...
static void mmc_blk_mq_req_done(struct mmc_request *mrq)
{
	...
	if (host->card && mmc_card_sd(host->card)) {
		if ((mrq->cmd && (mrq->cmd->error == -ETIMEDOUT)) || (mrq->stop
				&& (mrq->stop->error == -ETIMEDOUT)) || (mrq->sbc
						&& (mrq->sbc->error == -ETIMEDOUT)))
			sdinfo.cmd_timeout_count += 1;

		else if ((mrq->cmd && (mrq->cmd->error == -EILSEQ)) || (mrq->stop
				&& (mrq->stop->error == -EILSEQ)) || (mrq->sbc && (mrq->sbc->error == -EILSEQ)))
...
char *capacity_string(struct mmc_card *card)
{
	...
	string_get_size((u64)get_capacity(md->disk), 512, STRING_UNITS_2, cap_str,
			sizeof(cap_str));
	return cap_str;
}
#endif
