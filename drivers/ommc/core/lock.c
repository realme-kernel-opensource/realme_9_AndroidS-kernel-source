...
static void save_unlock_password(struct mmc_card *card, u8 *key_buffer,
				 int key_len)
{
	if (!card)
		return;
...
/* MMC_SUCCESS(0)  -- Success  or Failed */
static int sd_lock_do_assign(struct mmc_card *card, struct device *dev,
			     u8 *key_buf, int key_len)
{
	int ret = -EINVAL;
	...
	ret = mmc_lock_unlock_by_buf(card,  key_buf, key_len,
				     MMC_LOCK_MODE_LOCK | MMC_LOCK_MODE_SET_PWD);
...
/* MMC_SUCCESS(0)  -- Success  or Failed */
static int sd_lock_do_remove(struct mmc_card *card, struct device *dev,
			     u8 *key_buffer, int key_len)
{
	int ret = -EINVAL;
	int is_lock_before;
...
/* MMC_SUCCESS(0)  -- Success  or Failed */
static int sd_lock_do_lock(struct mmc_card *card, struct device *dev,
			   u8 *key_buffer, int key_len)
{
	int ret = -EINVAL;
...
/* MMC_SUCCESS(0)  -- Success  or Failed */
static int sd_lock_do_unlock(struct mmc_card *card, struct device *dev,
			     u8 *key_buffer, int key_len)
{
	int ret = -EINVAL;
