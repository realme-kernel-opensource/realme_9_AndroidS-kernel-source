			    ...
			    u8 num)
{
	...
	ret = gg_emulator_read_ext(host, card_status, read_status, ggc->_cur_read_buf,
				   512);
	...
	for (get_idx = 0; get_idx < num; get_idx++)
		(gg_reg_arr + get_idx)->value = _read_status_data_read_register(
							ggc->_cur_read_buf, (gg_reg_arr + get_idx));
...
bool gg_emulator_write_ext(struct sdhci_host *host, bool *card_status, u8 *data,
			   u32 datalen)
{
	bool ret = FALSE;
	bool wr_ret = FALSE;
...
bool ggc_write_registers_ext(struct sdhci_host *host, bool *card_status,
			     t_gg_reg_strt *gg_reg_arr, int num)
{
	u8 data[512];
...
bool ggc_set_output_tuning_phase_ext(struct sdhci_host *host, bool *card_status,
				     int sela, int selb)
{
	bool ret = TRUE;
	u8 data[512] = { 0 };
...
void tx_selb_failed_tb_reset(struct sdhci_host *host)
{
	...
	os_memset(&vendor_host->ggc.sdr104.tx_selb_tb, 0xff,
		  sizeof(vendor_host->ggc.sdr104.tx_selb_tb));
	os_memset(&vendor_host->ggc.sdr50.tx_selb_tb, 0xff,
...
void all_selb_failed_tb_reset(struct sdhci_host *host)
{
	...
	os_memset(vendor_host->ggc.sdr104.all_selb_tb, 0xff,
		  sizeof(vendor_host->ggc.sdr104.all_selb_tb));
	os_memset(vendor_host->ggc.sdr50.all_selb_tb, 0xff,
...
void chk_phase_window(u8 *tuning_win, u8 *mid_val,
		      u8 *max_pass_win)	// type[0] = 0: if 011110, right 1 index is the middle value, type[1] = 1: first_i valid
{
...
void chk_arr_max_win(u8 *tuning_win, u8 first_i, u8 *mid_val, u8 *first_val,
		     u8  *max_pass_win, chk_type_t
		     type)	// type[0] = 0: if 011110, right 1 index is the middle value, type[1] = 1: first_i valid
{
...
static int _tx_selb_inject_policy(int tx_selb, int org_selb)
{
	...
	if (1) {
		...
		for (i = ggc_get_01case_1_index(org_selb); i < TUNING_PHASE_SIZE && i >= 0
				&& group_cnt < TUNING_PHASE_SIZE;) {
...
void tx_selb_inject_policy(struct sdhci_host *host, int tx_selb)
{
	...
	vendor_host->ggc.ggc_cmd_tx_selb_failed_range = _tx_selb_inject_policy(tx_selb,
			vendor_host->ggc.ggc_cmd_tx_selb_failed_range);
	tx_selb_failed_history_update(host,
				      vendor_host->ggc.ggc_cmd_tx_selb_failed_range);
	PrintMsg("BHT MSG:after inject %xh range:%xh\n", tx_selb,
		 vendor_host->ggc.ggc_cmd_tx_selb_failed_range);

	if (is_bus_mode_sdr104(host))
		vendor_host->ggc.sdr104.fail_phase =
			vendor_host->ggc.ggc_cmd_tx_selb_failed_range;

	else
		vendor_host->ggc.sdr50.fail_phase =
			vendor_host->ggc.ggc_cmd_tx_selb_failed_range;
}
...
bool ggc_hw_inject_ext(struct sdhci_host *host, bool *card_status, u32 sel200,
		       u32 sel100, bool writetobh201)
{
	...
	get_gg_reg_cur_val(data);
	os_memcpy(&gg_reg_arr[0], &inject_failure_for_tuning_enable_cfg,
		  sizeof(t_gg_reg_strt));
	os_memcpy(&gg_reg_arr[1], &inject_failure_for_200m_tuning_cfg,
		  sizeof(t_gg_reg_strt));
	os_memcpy(&gg_reg_arr[2], &inject_failure_for_100m_tuning_cfg,
		  sizeof(t_gg_reg_strt));
...
{
	{
		...
		struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
		struct sdhci_msm_host *vendor_host = pltfm_host->priv;
		ret = ggc_hw_inject_ext(host, &card_status,
					vendor_host->ggc.ggc_cmd_tx_selb_failed_range,
		...
		if ((FALSE == ret) && (FALSE == card_status)) {
			...
			_ggc_update_cur_setting_for_sw_selb_tuning(host,
					vendor_host->ggc.ggc_cmd_tx_selb_failed_range); //update cur
			  ...
			  int  *dll_voltage_scan_map)
{
	...
	DbgErr("BHT ERR:dll_voltage_scan_map:%x %x %x %x\n", dll_voltage_scan_map[0],
	       dll_voltage_scan_map[1],
...
bool _ggc_calc_cur_sela_tuning_result(struct sdhci_host *host, int cur_sela,
				      int start_selb)
{
	...
	ret = ggc_sw_calc_tuning_result(host, &card_status, &read_status, &tx_selb,
					&all_selb, &raw_tx_selb);

	if (FALSE == card_status) {
		if (TRUE == read_status) {
			int selb = get_selb_failure_point(start_selb, raw_tx_selb,
		...
		if (TRUE == read_status) {
			...
			if (selx_failure_point_exist(tx_selb)) {
				...
				//update ggc setting
				_ggc_update_cur_setting_for_sw_selb_tuning(host,
						ggc->ggc_cmd_tx_selb_failed_range); //update cur
...
#define GGC_SW_SELB_TUNING_LOOP 3
	...
	rescan_selb:
		...
		if (dll_sela_cnt == 0) {
...
#ifdef FORCE_FIRST_CMD19_FAIL_ERROR
			{
				...
				if (dll_vol_change_test_flag) {
					PrintMsg("BHT MSG:force first cmd19 fail to debug dll voltage change, cur_dll_voltage_idx=%d",
			...
			if (FALSE == first_cmd19_sta) {
				...
				if (TRUE == get_next_dll_voltage(ggc->cur_dll_voltage_idx, & next,
								 ggc->cur_bus_mode->dll_voltage_unlock_cnt, ggc->dll_voltage_scan_map))
...
#ifdef FORCE_DRIVER_STRENGTH_ERROR
		{
			...
			while (driver_strength_reinit_test_flag
					&& get_bit_number(vendor_host->ggc.ggc_cmd_tx_selb_failed_range) > 6) {
	...
	cur_sela_failed:
		...
		all_selb_failed_tb_update(host, cur_sela,
					  all_selb);	//mark failed case to failed.
