			   ...
			   struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
			"echo 1 > /sys/devices/platform/soc/externdevice/sdcard_remove\nremove_sdcard=%d\n",
			remove_sdcard);
}
			    ...
			    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value) {
		...
		case RESET_SDCARD_QUICK_RETURN :
			schedule_delayed_work(&remove_sdcard_reset_work, msecs_to_jiffies(1000));
			break;
...
void set_sdinfo(struct mmc_card *card)
{
	...
	sdinfo.supported_bus_mode = card->sw_caps.sd3_bus_mode &
				    (SD_MODE_UHS_SDR104 | SD_MODE_UHS_SDR50 | SD_MODE_UHS_DDR50 | SD_MODE_UHS_SDR25
				     | SD_MODE_UHS_SDR12);
