...
{
	struct o_ufsplus_status *ufsplus_status  = (struct o_ufsplus_status *)
			s->private;
				     ...
				     int *tw_status)
{
	...
	ufsplus_status = (struct o_ufsplus_status *)kzalloc(sizeof(*ufsplus_status),
			 GFP_KERNEL);
...
reinit_aboard_id_for_brandon(struct device *dev, struct device_info *dev_info)
{
	...
	if ((2 == operate) || (8 == operate)) {
		...
		if (((adc_value >= 570) && (adc_value <= 690)) || ((adc_value >= 410)
				&& (adc_value <= 520)))
			ret = 0;
...
reinit_aboard_id(struct device *dev, struct manufacture_info *info)
{
	...
	ignore_size = of_property_count_elems_of_size(np, "devinfo-match-ignore-list",
			sizeof(uint32_t));
	...
	if (ignore_size > 0) {
		...
		of_property_read_u32_array(np, "devinfo-match-ignore-list", ignore_list,
					   ignore_size);
...
static int __attribute__((__unused__)) init_ddr_type(struct device_info
		*dev_info)
{
