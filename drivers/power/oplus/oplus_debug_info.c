...
struct vooc_charge_strategy
	*g_vooc_charge_strategy_p[OPLUS_FASTCHG_OUTPUT_POWER_MAX] = {
	NULL,
	NULL,
	NULL,
	NULL,
...
static void oplus_chg_send_info_dwork(struct work_struct *work)
{
	...
	if ((ret > 0) && (oplus_chg_debug_info.retry_cnt > 0)) {
		...
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_CHG_SLOW_BATT_NON_AUTH,
						 OPLUS_NOTIFY_GAUGE_UNSEAL_FAIL);/*ic error*/
		...
		if (oplus_switching_support_parallel_chg() == 1) {
			oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_PARALLEL_LIMITIC_ERROR,
...
static void oplus_chg_print_debug_info(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_debug_info.notify_type ||
		...
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				"$$adapter_type@@0x%x", (oplus_chg_debug_info.fast_chg_type << 8) |
		...
		if ((oplus_chg_debug_info.notify_type & (1 <<
		...
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				...
				chip->charging_state, oplus_chg_debug_info.total_time,
				...
				chip->stop_voter, chip->notify_code, chip->sw_full, chip->hw_full_by_sw,
				chip->hw_full,
		...
		if (oplus_switching_support_parallel_chg()) {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					...
					oplus_switching_get_hw_enable(), oplus_switching_get_charge_enable(),
		...
		if (vooc_chip) {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					"VOOC[ %d / %d / %d / %d / %d / %d]",
					vooc_chip->fastchg_allow, vooc_chip->fastchg_started,
...
{
	int i;
	int arr_size = sizeof(vooc_charger_type_list) / sizeof(
			       vooc_charger_type_list[0]);
...
static int oplus_chg_get_vooc_adapter_is_low_input_power(
...
static int oplus_chg_get_vooc_input_power(struct oplus_chg_chip *chip)
{
	...
	if (vooc_chip != NULL) {
		if (vooc_chip->vooc_chg_current_now > 0)
			cur = cur < vooc_chip->vooc_chg_current_now ? cur :
		...
		if (ret > 0 && ret < (array_len + 1))
			cur = cur < multistepCurrent_debug[ret - 1] ? cur : multistepCurrent_debug[ret -
					1];
	}
	...
	if (oplus_chg_debug_info.vooc_charge_strategy) {
		...
		mcu_chg_current =
			oplus_chg_debug_info.vooc_charge_strategy->input_current[oplus_chg_debug_info.vooc_charge_input_current_index].cur;
...
static int oplus_chg_get_charge_power(struct oplus_chg_chip *chip)
{
	...
	if (chg_check_point_debug & OPEN_LOG_BIT) {
		chg_err("charge_pwoer: %d icharging = %d batt_volt = %d\n", charge_power,
			chip->icharging, chip->batt_volt);//debug tmp
	}
...
static int oplus_chg_set_chg_flag(int index)
{
	...
	case OPLUS_CHG_NOTIFY_TOTAL:
		if (((oplus_chg_debug_info.chg_cnt[index] % (policy_time *
				oplus_chg_debug_notify_policy[index].percent / 100)) == 0))
			oplus_chg_debug_info.notify_flag |= (1L << index);
...
static int oplus_chg_vooc_charge_strategy_init(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_debug_info.vooc_charge_strategy != NULL) {
		for (i = oplus_chg_debug_info.vooc_charge_input_current_index;
			...
			if (chip->batt_volt <=
					oplus_chg_debug_info.vooc_charge_strategy->input_current[i].vol) {
...
static int oplus_chg_normal_chg_consume_power_check(struct oplus_chg_chip *chip)
{
	...
	if (!chip->led_on) {
		if ((chip->total_time - oplus_chg_debug_info.led_off_start_time) >
				LED_OFF_STABLE_TIME) {
...
static int oplus_parallel_full_soc_error_check(struct oplus_chg_chip *chip)
{
	...
	if ((chg_status == POWER_SUPPLY_STATUS_FULL) && (chip->sub_batt_soc < 97
			|| chip->soc < 97)) {
	...
	if (oplus_chg_chg_is_normal_path(chip)) {
		...
		if (chip->batt_volt < float_voltage) {
			...
			if ((default_input_current > 0) && (max_charger_voltage > 0)) {
				if ((-1 * charge_power * 10) < (default_input_current * max_charger_voltage /
								1000))
					oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_LOW_CHARGE_CURRENT_LONG_TIME);
			}
		}
...
static int oplus_get_chg_time_ref(struct oplus_chg_chip *chip)
{
	...
	chg_err("speed_high:%d speed_mid:%d speed_low:%d speed_lowest:%d\n", speed_high,
		speed_mid, speed_low, speed_lowest); //debug temp
...
static int oplus_get_chg_slow_reason(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_debug_notify_flag_is_set(OPLUS_NOTIFY_CHG_SLOW_BATT_WARM_TEMP) ==
			1) {
	...
	if (oplus_chg_debug_notify_flag_is_set(OPLUS_NOTIFY_CHG_SLOW_BATT_COLD_TEMP) ==
			1) {
		oplus_chg_set_notify_type(OPLUS_CHG_DEBUG_NOTIFY_TYPE_CHG_SLOW);
		strcpy(oplus_chg_debug_info.flag_reason,
...
static int oplus_chg_chg_slow_check(struct oplus_chg_chip *chip)
{
	...
	oplus_chg_debug_info.pre_prop_status = chip->prop_status;
	oplus_chg_debug_info.chg_total_time = oplus_chg_debug_info.total_time -
	...
	if (!oplus_chg_chg_is_normal_path(chip)) {
		if (chip->cool_down > 0 && chip->led_on)
			oplus_chg_debug_info.cool_down_by_user = 1;//modify for none mcu svooc
...
{
	if (oplus_chg_debug_info.soc_load_flag & (1 <<
...
static int oplus_chg_chg_batt_capacity_jump_check(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_debug_info.cur_soc == OPLUS_CHG_BATT_INVALID_CAPACITY) {
		...
		if (abs(oplus_chg_debug_info.cur_soc - chip->soc_load) >
			...
			oplus_chg_debug_info.soc_load_flag |= 1 <<
							      OPLUS_NOTIFY_BATT_SOC_CAPCITY_LOAD_JUMP;
...
static int oplus_chg_debug_info_parse_dt(struct oplus_chg_chip *chip)
{
	...
	for (i = 0; i < OPLUS_FASTCHG_OUTPUT_POWER_MAX; i++) {
		...
		g_vooc_charge_strategy_p[i] = kzalloc((count + 1) * sizeof(
				struct vooc_charge_strategy), GFP_KERNEL);
		...
		for_each_child_of_node(node, child) {
			...
			if (size >= 0) {
				for (k = 0; (k < (size / 3))
						&& (k < OPLUS_CHG_VOOC_INPUT_CURRENT_MAX_CNT); k++) {
...
#ifdef CONFIG_OPLUS_FEATURE_FEEDBACK
	oplus_chg_debug_info.dcs_info = (struct kernel_packet_info *)
					&oplus_chg_debug_msg[0];
