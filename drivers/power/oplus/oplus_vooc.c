...
int __attribute__((weak)) request_firmware_select(const struct firmware
		**firmware_p,
...
static void check_charger_out_work_func(struct work_struct *work)
{
	...
	struct oplus_vooc_chip *chip = container_of(dwork, struct oplus_vooc_chip,
...
static int oplus_vooc_set_current_1_temp_normal_range(struct oplus_vooc_chip
		*chip, int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_vooc_set_current_temp_low_normal_range(
...
static int oplus_vooc_set_current_temp_little_cool_range(
	struct oplus_vooc_chip *chip, int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_vooc_set_current_temp_little_cold_range(
	struct oplus_vooc_chip *chip, int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_vooc_set_current_2_temp_normal_range(struct oplus_vooc_chip
		*chip, int vbat_temp_cur)
{
	int ret = 0;
...
int oplus_vooc_get_smaller_battemp_cooldown(int ret_batt, int ret_cool)
{
	...
	if (ret_batt > 0 && ret_batt < (array_len + 1)
		...
		ret_cool_current = ret_cool_current < ret_batt_current ? ret_cool_current :
				   ret_batt_current;
...
int vooc_hardware_init_cp(int data)
{
	...
	if (data == 0 || data == 0x13 || data == 0x19 || data == 0x29 || data == 0x34
			|| data == 0x41
	...
	} else if (data == 0x11 || data == 0x12 || data == 0x21 || data == 0x31
			|| data == 0x33
			|| data == 0x14 || data == 0x61 || data == 0x66 || data == 0x69
			|| data == 0x6e) {
...
static void oplus_vooc_fastchg_func(struct work_struct *work)
{
	...
	*/
	...
	if ((data_head != 0x50 && data_head != 0x70) && (!adapter_fw_ver_info)
	...
	if (data == VOOC_NOTIFY_FAST_PRESENT) {
		...
		if (chip->hiz_gnd_cable)
			chip->allowed_current_max = oplus_get_allowed_current_max(
		...
		if (chip->vooc_multistep_adjust_current_support
			...
			if (select_func_flag == 1)
				ret_info_temp = oplus_vooc_set_current_when_bleow_setting_batt_temp(chip,
						temp);
		...
		if (chip->hiz_gnd_cable) {
#endif
			ret_rst = oplus_vooc_get_smaller_battemp_cooldown(chip->allowed_current_max,
					ret_info);
		...
		} else if ((chip->vooc_multistep_adjust_current_support == true)
				&& (soc > 75)) {
			ret_rst = oplus_vooc_get_smaller_battemp_cooldown(pre_ret_info , ret_info);
		...
		vooc_xlog_printk(CHG_LOG_CRTI, "temp_range[%d-%d-%d-%d-%d-%d]",
				 chip->vooc_low_temp, chip->vooc_little_cold_temp,
				 chip->vooc_cool_temp, chip->vooc_little_cool_temp, chip->vooc_normal_low_temp,
				 chip->vooc_normal_high_temp, chip->vooc_high_temp);
		vooc_xlog_printk(CHG_LOG_CRTI,
		...
		if (g_adapter_chip->adapter_firmware_data[g_adapter_chip->adapter_fw_data_count
				- 4] > data
				&& (oplus_gauge_get_batt_soc() > 2)
				&& (chip->vops->is_power_off_charging(chip) == false)
...
static int init_vooc_proc(struct oplus_vooc_chip *chip)
{
	...
	if (get_vooc_mcu_type(chip) == OPLUS_VOOC_MCU_HWID_STM8S)
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/%d/oplus_vooc_fw.bin",
			 get_project());

	else if (get_vooc_mcu_type(chip) == OPLUS_VOOC_MCU_HWID_N76E)
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/%d/oplus_vooc_fw_n76e.bin",
			 get_project());
...
void oplus_vooc_init(struct oplus_vooc_chip *chip)
{
	...
	if (chip->vooc_fw_update_newmethod) {
		...
		chip->manufacture_info.manufacture = kzalloc(MAX_DEVICE_MANU_LENGTH,
						     GFP_KERNEL);
...
void oplus_vooc_print_log(void)
{
	...
	vooc_xlog_printk(CHG_LOG_CRTI, "VOOC[ %d / %d / %d / %d / %d / %d]\n",
			 g_vooc_chip->fastchg_allow, g_vooc_chip->fastchg_started,
			 g_vooc_chip->fastchg_dummy_started,
			 g_vooc_chip->fastchg_to_normal, g_vooc_chip->fastchg_to_warm,
