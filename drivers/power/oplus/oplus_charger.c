...
static void battery_notify_charge_batt_terminal_check(struct oplus_chg_chip
		*chip);
...
static int oplus_chg_voocphy_get_cp_vbat_deviation(void)
{
	...
	vbat_deviation = (gauge_bat > cp_vbat) ? gauge_bat - cp_vbat : cp_vbat -
			 gauge_bat;
			       ...
			       const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
		...
		schedule_delayed_work(&g_charger_chip->update_work,
...
static int oplus_chg_update_slow(struct oplus_chg_chip *chip)
{
	...
	slow = !((chip->wireless_support
		  && oplus_wpc_get_wireless_charge_start() == true) || chip->charger_exist ||
	...
	if (!first_pass) {
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY
			...
			while (retry > 0) {
				...
				gauge_bat = oplus_gauge_get_batt_mvolts();
				vbat_deviation = (gauge_bat > plat_vbat) ? gauge_bat - plat_vbat : plat_vbat -
						 gauge_bat;
					...
					char __user *buff, size_t count, loff_t *off)
{
	...
	if (chip->vooc_show_ui_soc_decimal) {
		svooc_is_control_by_vooc = (chip->vbatt_num == 2
					    && oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC);
#ifndef CONFIG_OPLUS_CHARGER_MTK
...
void oplus_chg_ui_soc_decimal_init(void)
{
	...
	if (chip->ui_soc == 100) {
		...
		chip->ui_soc_integer =  chip->ui_soc  * 1000;
		chip->ui_soc_decimal = chip->batt_rm * 100000 / chip->batt_fcc -
...
void oplus_chg_ui_soc_decimal_deinit(void)
{
	...
	pr_err("[oplus_chg_ui_soc_decimal_deinit] ui_soc:%d, chip->soc[%d], chip->ui_soc_decimal[%d]",
	       chip->ui_soc, chip->soc, chip->ui_soc_decimal);
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
	...
	chip->shortc_thread = kthread_create(shortc_thread_main, (void *)chip,
					     thread_name);
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	chip->limits.iterm_disabled = of_property_read_bool(node,
				      "qcom,iterm_disabled");
	...
	chip->smart_charging_screenoff = of_property_read_bool(node,
	...
	rc = of_property_read_u32(node,
				  "qcom,pd_temp_little_cold_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,pd_temp_little_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,qc_temp_little_cold_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,qc_temp_little_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,pd_temp_little_cool_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,pd_temp_little_cool_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,qc_temp_little_cool_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,qc_temp_little_cool_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,temp_normal_phase2_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,temp_normal_phase2_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,temp_normal_phase3_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,temp_normal_phase3_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,temp_normal_phase4_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,temp_normal_phase4_fastchg_current_ma_low",
	...
	chip->vooc_enable_charger_first = of_property_read_bool(node,
	...
	rc = of_property_read_u32(node,
				  "qcom,default_temp_little_cool_fastchg_current_ma",
	...
	chip->chg_ctrl_by_vooc_default = of_property_read_bool(node,
	...
	chip->limits.default_qc_input_current_charger_ma
		= chip->limits.qc_input_current_charger_ma;
	chip->suspend_after_full = of_property_read_bool(node,
				   "qcom,suspend_after_full");
	chip->check_batt_full_by_sw = of_property_read_bool(node,
				      "qcom,check_batt_full_by_sw");
	chip->external_gauge = of_property_read_bool(node, "qcom,external_gauge");
	chip->external_authenticate = of_property_read_bool(node,
	...
	chip->wpc_no_chargerpump = of_property_read_bool(node,
				   "qcom,wpc_no_chargerpump");
	chip->chg_ctrl_by_lcd = of_property_read_bool(node, "qcom,chg_ctrl_by_lcd");
	chip->chg_ctrl_by_lcd_default = of_property_read_bool(node,
					"qcom,chg_ctrl_by_lcd");
	chip->chg_ctrl_by_camera = of_property_read_bool(node,
				   "qcom,chg_ctrl_by_camera");
	chip->bq25890h_flag = of_property_read_bool(node, "qcom,bq25890_flag");
	chip->chg_ctrl_by_calling = of_property_read_bool(node,
				    "qcom,chg_ctrl_by_calling");
	chip->ffc_support = of_property_read_bool(node, "qcom,ffc_support");
	chip->dual_ffc = of_property_read_bool(node, "qcom,dual_ffc");
	chip->fg_info_package_read_support =  of_property_read_bool(node,
					      "qcom,fg_info_package_read_support");
	chip->new_ui_warning_support = of_property_read_bool(node,
	...
	chip->efttest_fast_switch = of_property_read_bool(node,
	...
	chip->dual_charger_support = of_property_read_bool(node,
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			    "chip->usbtemp_temp_up_time_thr=%d, chip->usbtemp_max_temp_thr=%d, chip->usbtemp_batttemp_gap[%d]\n",
	...
	chip->vooc_show_ui_soc_decimal = of_property_read_bool(node,
	...
	chip->support_abnormal_adapter = of_property_read_bool(node,
...
#ifdef OPLUS_CUSTOM_OP_DEF
	chip->hiz_gnd_cable = of_property_read_bool(node,
			      "qcom,supported_hiz_gnd_cable");
#endif

	chip->support_low_soc_unlimit = of_property_read_bool(node,
					"qcom,support_low_soc_unlimit");
	rc = of_property_read_u32(node, "qcom,unlimit_soc", &chip->unlimit_soc);

	if (rc)
		chip->unlimit_soc = 0;

	chip->nightstandby_support = of_property_read_bool(node,
				     "qcom,nightstandby_support");
	chip->screen_off_control_by_batt_temp = of_property_read_bool(node,
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on) &&
		...
		if (chip->dual_charger_support
				&& chip->tbatt_normal_status == BATTERY_STATUS__NORMAL_PHASE6)
			charging_current = chip->limits.temp_warm_fastchg_current_ma_led_on;
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
		if (!chip->dual_charger_support || (chip->dual_charger_support
...
static void oplus_chg_check_vbatt_higher_than_4180mv(struct oplus_chg_chip
		*chip)
{
...
static void battery_temp_normal_anti_shake_handle(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_normal_status == BATTERY_STATUS__NORMAL_PHASE1) {
		...
		chip->limits.normal_phase2_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase2_bound + high_shake;
		chip->limits.normal_phase3_bat_decidegc =
		...
		chip->limits.normal_phase1_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase1_bound;
		chip->limits.normal_phase2_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase2_bound + low_shake;
		chip->limits.normal_phase3_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase3_bound + high_shake;
		chip->limits.normal_phase4_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase4_bound;
		chip->limits.normal_phase5_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase5_bound;
		...
		chip->limits.normal_phase3_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase3_bound + low_shake;
		chip->limits.normal_phase4_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase4_bound + high_shake;
		chip->limits.normal_phase5_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase5_bound;
		chip->limits.normal_phase6_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase6_bound;
		...
		chip->limits.normal_phase3_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase3_bound;
		chip->limits.normal_phase4_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase4_bound + low_shake;
		chip->limits.normal_phase5_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase5_bound + high_shake;
		chip->limits.normal_phase6_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase6_bound;
		...
		chip->limits.normal_phase4_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase4_bound;
		chip->limits.normal_phase5_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase5_bound + low_shake;
		chip->limits.normal_phase6_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase6_bound + high_shake;
		...
		chip->limits.normal_phase5_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase5_bound;
		chip->limits.normal_phase6_bat_decidegc =
			chip->tbatt_normal_anti_shake_bound.phase6_bound + low_shake;
...
static void oplus_chg_check_tled_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->temperature >
		...
		if (high_counts >= TLED_CHANGE_COUNTS) {
			...
			chip->limits.led_warm_bat_decidegc_antishake) {	/* >35C && <= 37 */
		warm_counts ++;
...
static void oplus_chg_check_vooc_temp_status(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.vooc_high_bat_decidegc) {			/*>45C*/
		if (oplus_vooc_get_fastchg_started() == true) {
			...
			chip->limits.vooc_warm_bat_decidegc_antishake) {	/*>38C && <= 45*/
		high_counts ++;
		...
		if (high_counts >= TVOOC_COUNTS
			...
			chip->limits.vooc_normal_bat_decidegc_antishake) { /*>34C && <= 38*/
		warm_counts ++;
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (oplus_switching_support_parallel_chg()) {
		...
		chg_err("sub_batt volt:%d, current:%d, soc:%d, temperature:%d\n",
			chip->sub_batt_volt, chip->sub_batt_icharging, chip->soc,
			chip->sub_batt_temperature);
	}
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if ((chip->charger_exist || oplus_wpc_get_wireless_charge_start())
			&& chip->batt_exist && chip->batt_full && chip->mmi_chg && (chip->stop_chg == 1
					|| chip->charger_type == 5)) {
#else
	...
	if (chip->charger_exist && chip->batt_exist && chip->batt_full && chip->mmi_chg
			...
			if (soc_down_count >= soc_down_limit) {
				...
				if ((soc_pre > chip->soc || soc_pre == 0 || allow_uisoc_down)
					...
					chg_debug("chg [soc ui_soc smooth_soc soc_pre down_limit up_limit] = [%d %d %d %d %d %d]\n",
						  chip->soc, chip->ui_soc, chip->smooth_soc, soc_pre, soc_down_limit,
						  soc_up_limit);
				}
			}
		}
		...
		if (chip->smooth_soc <= chip->ui_soc || vbatt_too_low) {
			...
			if (soc_down_count >= soc_down_limit && (chip->smooth_soc < chip->ui_soc
					|| vbatt_too_low)) {
				chip->sleep_tm_sec = 0;
				soc_down_count = 0;
				chip->ui_soc--;
...
static int oplus_chg_check_sw_sub_batt_full(struct oplus_chg_chip *chip)
{
	...
	if (chip->sub_batt_volt > sub_vbatt_full_vol_sw) {
		if (chip->sub_batt_icharging < 0
				&& (chip->sub_batt_icharging * -1) <= chip->limits.sub_iterm_ma) {
...
static bool oplus_chg_check_sub_ibat_ffc_end(struct oplus_chg_chip *chip)
{
	...
	if (chip->fastchg_ffc_status == 1) {
		if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL)
			ffc_ibatt_sw_limit = chip->limits.ff1_normal_fastchg_ma -
					     chip->limits.sub_ff1_exit_step_ma;

		else if (chip->ffc_temp_status == FFC_TEMP_STATUS__WARM)
			ffc_ibatt_sw_limit = chip->limits.ff1_warm_fastchg_ma -
					     chip->limits.sub_ff1_warm_exit_step_ma;
		...
		if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL)
			ffc_ibatt_sw_limit = chip->limits.ffc2_normal_fastchg_ma -
					     chip->limits.sub_ffc2_exit_step_ma;

		else if (chip->ffc_temp_status == FFC_TEMP_STATUS__WARM)
			ffc_ibatt_sw_limit = chip->limits.ffc2_warm_fastchg_ma -
					     chip->limits.sub_ffc2_warm_exit_step_ma;
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	if (chip->vbatt_num == 1) {
		...
		charger_xlog_printk(CHG_LOG_CRTI,
				    ...
				    chip->otg_online, chip->otg_switch, chip->mmi_chg, chip->boot_reason,
				    chip->boot_mode,
				    chip->chargerid_volt, chip->chargerid_volt_got, chip->shell_temp,
	...
	if (chip->charger_exist) {
		if (chgr_dbg_total_time != 0)
			...
			if (!chip->total_time) {
				...
				chip->total_time = OPLUS_CHG_UPDATE_INTERVAL_SEC + cur_chg_time -
						   start_chg_time;
			}
		}
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		if ((chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr
				...
				(chip->limits.tbatt_pdqc_to_5v_thr > 0
				 && chip->temperature > chip->limits.tbatt_pdqc_to_5v_thr)) && pdqc_9v == true) {
...
int oplus_chg_get_cool_down_status(void)
{
	...
	if (chip->smart_charge_user == SMART_CHARGE_USER_USBTEMP) {
		if (chip->smart_charging_screenoff)
			chip->usbtemp_cool_down = oplus_convert_current_to_level(chip,
		...
		if (chip->cool_down)
			ret = (chip->usbtemp_cool_down < chip->cool_down) ? chip->usbtemp_cool_down :
			      chip->cool_down;
...
void oplus_smart_charge_by_shell_temp(struct oplus_chg_chip *chip, int val)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			    "val->intval = [%04x], shell_temp = [%04x], set shell_temp_down = [%d] pre_shell_temp_current =[%d] .\n",
	...
	if (!val) {
		if (c_level_index <= 0) {
			...
			if (chip->vbatt_num == 1) {
				chip->limits.pd_input_current_charger_ma = choose_little_current(
							cool_down_current_limit_onebat[onebat_index_temp - 1],
							chip->limits.default_pd_input_current_charger_ma);
				chip->limits.qc_input_current_charger_ma = choose_little_current(
							cool_down_current_limit_onebat[onebat_index_temp - 1],
							chip->limits.default_qc_input_current_charger_ma);
	...
	case CHARGER_SUBTYPE_FASTCHG_SVOOC:
		...
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY
			...
			if (c_level_index > 0) {
				...
				chip->cool_down = chip->cool_down < c_level_index ? chip->cool_down :
						  c_level_index;
	...
	case CHARGER_SUBTYPE_FASTCHG_VOOC:
		if (chip->chg_ctrl_by_vooc == true) {
			if (c_level_index <= 0) {
				...
				if (chip->vbatt_num == 1)
					chip->limits.input_current_cool_down_ma = val <
							cool_down_current_limit_onebat[onebat_index_temp - 1] ? val :
							cool_down_current_limit_onebat[onebat_index_temp - 1];

				else
					chip->limits.input_current_cool_down_ma = val <
							cool_down_current_limit_normal[normal_index_temp - 1] ? val :
							cool_down_current_limit_normal[normal_index_temp - 1];
				...
				chip->limits.qc_input_current_charger_ma =
					chip->limits.input_current_cool_down_ma;
				chip->limits.input_current_vooc_ma_high = val <
						cool_down_current_limit_vooc[vooc_index_temp - 1] ? val :
						cool_down_current_limit_vooc[vooc_index_temp - 1];
				chip->limits.input_current_vooc_ma_warm =
					chip->limits.input_current_vooc_ma_high;
				chip->limits.input_current_vooc_ma_normal =
			...
			if (c_level_index > 0)
				chip->cool_down = oplus_convert_current_to_level(chip,
						  val) < c_level_index ? oplus_convert_current_to_level(chip,
								  val) : c_level_index;
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			    "val = [%d],  subtype = [%d],vbatt_num = %d,c_level_index = %d,qc_input = %d,input_current = %d\n",
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	onebat_index_temp = ARRAY_SIZE(cool_down_current_limit_onebat) < val ?
			    ARRAY_SIZE(cool_down_current_limit_onebat) : val;
	normal_index_temp = ARRAY_SIZE(cool_down_current_limit_normal) < val ?
			    ARRAY_SIZE(cool_down_current_limit_normal) : val;
	vooc_index_temp = ARRAY_SIZE(cool_down_current_limit_vooc) < val ? ARRAY_SIZE(
				  cool_down_current_limit_vooc) : val;
...
#endif
	charger_xlog_printk(CHG_LOG_CRTI,
			    "val->intval = [%04x], set cool_down = [%d],vbatt_num = %d,pd_input = %d,qc_input = %d,input_current = %d\n",
...
static int oplus_tbatt_power_off_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		schedule_timeout_interruptible(round_jiffies_relative(msecs_to_jiffies(
						       5 * 1000)));
...
static void get_time_interval(void)
{
	if (!g_charger_chip)
		...
		if ((g_charger_chip->second_ktime - g_charger_chip->first_ktime) >=
				TIME_INTERVALE_S) {
...
static void parallel_chg_set_balance_bat_status(int status)
{
	...
	if (chip->balancing_bat_status !=  status) {
		...
		case PARALLEL_NOT_NEED_BALANCE_BAT__START_CHARGE:
			...
			if (chip->charger_exist && chip->balancing_bat_status != -1
					&& chip->mmi_chg == 1)
				oplus_chg_turn_on_charging(chip);
...
static int parallel_chg_check_balance_bat_status(void)
{
	...
	for (i = 0; i < 3; i++) {
		status = oplus_switching_get_if_need_balance_bat(chip->batt_volt,
				chip->sub_batt_volt);
