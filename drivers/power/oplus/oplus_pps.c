...
static int oplus_pps_parse_batt_curves(struct oplus_pps_chip *chip)
{
	...
	for (i = 0; i < BATT_CURVE_SOC_MAX; i++) {
		for (j = 0; j < BATT_CURVE_TEMP_MAX; j++) {
			for (k = 0; k < chip->batt_curves_soc[i].batt_curves_temp[j].batt_curve_num;
					k++) {
				pr_err("%s: %d %d %d %d %d\n", __func__,
...
static int oplus_pps_get_status(struct oplus_pps_chip *chip)
{
	...
	chip->current_adapter_max = chip->get_pps_max_cur(oplus_pps_get_curve_vbus(
					    chip));
...
static int oplus_pps_choose_curves(struct oplus_pps_chip *chip)
{
	...
	memcpy(&chip->batt_curves,
	       chip->batt_curves_soc[batt_soc_plugin].batt_curves_temp[batt_temp_plugin].batt_curves,
	...
	while (chip->get_pps_max_cur(oplus_pps_get_curve_vbus(chip)) <
...
static int oplus_pps_get_target_current(struct oplus_pps_chip *chip)
{
	...
	target_current_temp = chip->current_batt_curve < chip->current_batt_temp ?
			      chip->current_batt_curve : chip->current_batt_temp;
	target_current_temp = target_current_temp < chip->current_cool_down ?
			      target_current_temp : chip->current_cool_down;
