...
static int wireless_chg_init(struct op_chg_chip *chip)
{
	...
	chg_param->fastchg_fod_enable = of_property_read_bool(node,
...
static void fastchg_temp_check(struct op_chg_chip *chip)
{
	...
	if (def_curr_ua <= 0)
		...
		def_curr_ua = min(get_client_vote(chip->wlcs_fcc_votable, DEF_VOTER),
				  def_curr_ua);
...
static void wlchg_fastchg_restart_check(struct op_chg_chip *chip)
{
	...
	if (is_client_vote_enabled(chip->fastchg_disable_votable, FFC_VOTER) &&
			(chip->temperature < (ffc_chg->ffc_step[ffc_chg->max_step - 1].high_threshold -
					      BATT_TEMP_HYST))) {
...
retry:
	...
	if (cp_status & (CP_DWP | CP_SWITCH_OCP | CP_CRP | CP_VOUT_OVP | CP_CLP |
			 CP_VBUS_OVP)) {
...
static int wlchg_charge_status_process(struct op_chg_chip *chip)
{
	...
	case WPC_CHG_STATUS_WAITING_FOR_TX_INTO_FASTCHG:
		...
		chg_info("vol_set_ok=%d, charge_type=%d", chg_status->vol_set_ok,
...
static int wlchg_driver_probe(struct platform_device *pdev)
{
	...
	chip->ap_ctrl_dcdc = of_property_read_bool(chip->dev->of_node,
			     "op-ap_control_dcdc");
	...
	chip->reverse_wlchg_wake_lock = wakeup_source_register(chip->dev,
	...
	chip->fastchg_disable_votable = create_votable("WLCHG_FASTCHG_DISABLE",
					VOTE_SET_ANY,
