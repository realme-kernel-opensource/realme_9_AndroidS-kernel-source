...
static int __init __attribute__((unused)) oplus_short_c_battery_status_init(
	char *str)
{
...
static int __init __attribute__((unused)) oplus_short_c_switch_status_init(
	char *str)
{
...
static int __init __attribute__((unused)) oplus_short_c_feature_sw_status_init(
	char *str)
{
...
static int __init __attribute__((unused)) oplus_short_c_feature_hw_status_init(
	char *str)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
ssize_t __attribute__((weak)) vfs_read(struct file *file, char __user *buf,
				       size_t count, loff_t *pos)
{
	return -EINVAL;
}

ssize_t __attribute__((weak)) vfs_write(struct file *file,
		...
		int temp1, int temp2, int vbatt1, int vbatt2)
{
	...
	/*add ",#" and the end of the string for userspace*/
	sprintf(buf, "exit_code%d:%d,%d,%d,%d,#", exit_code, temp1, temp2, vbatt1,
		vbatt2);
	...
	if (chip->batt_full == true && chip->chging_on == false
		...
		if (full_timer3_count <= chip->short_c_batt.full_timer3) {
			...
			if (full_temp1 != INVALID_TEMP
					&& abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
