...
int bq2589x_set_boost_current(struct bq2589x *bq, int curr)
{
	...
	printk("bq2589x_set_boost_current temp=%d\n",temp);
...
static int bq2589x_inform_charger_type(struct bq2589x *bq)
{
	...
	pr_notice("inform power supply online status:%d\n", propval.intval);
...
static irqreturn_t bq2589x_irq_handler(int irq, void *data)
{
	...
	if (!prev_pg && bq->power_good) {
		...
		/*step3: BC1.2*/
		chg_err("adapter/usb inserted. start bc1.2");
		Charger_Detect_Init();
		bq2589x_force_dpdm(bq, true);
		...
		chg_err("wake up the chgtype thread.");
	...
	chg_err(" prev_chg_type %d --> cur_chg_type %d, bq->oplus_chg_type %d, chg_det_enable %d, ret %d\n",
	...
	if ((cur_chg_type == POWER_SUPPLY_TYPE_USB)
		...
		/*Step 6.2: DCP/HVDCP*/
		chg_err(" type cdp or dcp type = %d, vbus_type = %d\n", cur_chg_type, bq->vbus_type);
		Charger_Detect_Release();
...
static int bq2589x_init_device(struct bq2589x *bq)
{
	...
	if (bq->boot_mode == META_BOOT) {
		...
		schedule_delayed_work(&bq->enter_hz_work, msecs_to_jiffies(30000));
	}
...
int oplus_bq2589x_set_aicr(int current_ma)
{
	...
	if (g_bq->is_force_aicl) {
		chg_err("is_force_aicl current_ma = %d mA", current_ma);
		return 0;
	}
	...
	if (chip->is_double_charger_support) {
		...
		chg_err("disable subchg ");
	}

	chg_err("usb input max current limit=%d, em_mode = %d", current_ma, chip->em_mode);
	...
	chg_err("usb input max current limit=%d, aicl_point_temp=%d", current_ma, aicl_point_temp);
	if (current_ma < 500) {
		i = 0;
		goto aicl_end;
	}
...
aicl_pre_step:
	if ((chip->is_double_charger_support)
		...
		pr_info("usb input max current limit aicl: master and salve input current: %d, %d\n",
			main_cur, slave_cur);
	...
	pr_info("%s:usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step, slave:%d\n",
...
aicl_end:
	...
	pr_info("%s:usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_end,slave:%d\n",
		__func__, chg_vol, i, usb_icl[i], aicl_point_temp,
		chip->slave_charger_enable);
	return rc;
}
	...
	bool status = false;
        ...
        if (g_bq->tcpc) {
                if (tcpm_inquire_typec_attach_state(g_bq->tcpc) == TYPEC_ATTACHED_SRC){
                        pr_err("oplus_bq2589x_check_chrdet_status: OTG online.\n");
                        return false;
                }
        }
#endif
...
void vol_convert_work(struct work_struct *work)
{
	...
	if (!g_bq->pdqc_setup_5v) {
		if (mt6357_get_vbus_voltage() < 5800) {
			...
			while(retry--) {
				if (mt6357_get_vbus_voltage() > 7600) {
					chg_err("set_to_9v success\n");
					break;
				}
				msleep(50);
			}
			chg_err("set_to_9v complete, set is_force_aicl as false.\n");
			g_bq->is_force_aicl = false;
			bq2589x_disable_enlim(g_bq);
		}
	} else {
		if (mt6357_get_vbus_voltage() > 7500) {
			...
			while(retry--) {
				if (mt6357_get_vbus_voltage() < 5800) {
					chg_err("set_to_5v success\n");
					break;
				}
				msleep(50);
			}
			chg_err("set_to_5v complete, set is_force_aicl as false.\n");
			g_bq->is_force_aicl = false;
			bq2589x_disable_enlim(g_bq);
		}
	}
...
int oplus_bq2589x_set_qc_config(void)
{
	...
	if ( chip->cool_down_force_5v
		...
		if (mt6357_get_vbus_voltage() < 7500) {
			chg_err("start set to 9V. count (%d)", qc_to_9v_count);
...
{
	if (mt6357_get_vbus_status() && !g_oplus_chip->ac_online ) {
...
static void bq2589x_enter_hz_work_handler(struct work_struct *work) {
	pr_notice("[%s] enter hz mode for meta boot mode!\n", __func__);
	bq2589x_enter_hiz_mode(g_bq);
}
...
static void retry_detection_work_callback(struct work_struct *work)
{
	if (g_bq->sdp_retry || g_bq->cdp_retry || g_bq->retry_hvdcp_algo) {
		Charger_Detect_Init();
		pr_notice("usb/cdp start bc1.2 once\n");
		oplus_for_cdp();
...
static void aicr_setting_work_callback(struct work_struct *work)
{
	...
	//chg_err("begin aicr current = %d mA, wait %d mS", g_bq->aicr, re_check_count*20);
	...
	if (chip->is_double_charger_support) {
		if (chip->slave_charger_enable || chip->em_mode ) {
			...
			if ( !ret ) {
				...
				pr_err("[%s] set main_uA failed.\n");
			}
			slave_uA = uA - main_uA;
			if (chip->sub_chg_ops) {
				pr_info("set main_uA = %d, slave_uA = %d", main_uA, slave_uA);
				...
				if (ret) {
					pr_err("bq2589x set slave_uA:%d failed\n", slave_uA);
				}
			}
		} else {
		...
		ret = bq2589x_set_chargecurrent(g_bq, uA/1000);
		if (ret) {
		        pr_err("[%s] set main uA %d failed.\n", uA);
		}
	}

	chg_err(" current = %d, current_percent = %d, uA = %d, main_uA = %d, slave_uA = %d\n",
		cur, current_percent, uA, main_uA, slave_uA);
